...
public class ProductAction extends ActionSupport {
	...
	/**
	 * 销售记录查询列表——————接口 keyword,starttime,endTime,status
	 * 
	 * @throws Exception
	 */
		...
		try {
			...
			if (keyparam.equals(key)) {
				...
				for (Sell sell : list) {
					...
					Object sellVisitlist = productManager
							.findSellVisitList(sell);
					List<SellProduct> sellProductlist = (List<SellProduct>) productManager
							.findsellProductList(sell);

					for (SellProduct product : sellProductlist) {

						Product pro = productManager
								.findSellProductList(product);

						product.setProductName(pro.getProductName());
						product.setPrice(pro.getPrice());
					}

					Contacts contacts = productManager
							.findSellContactsList(sell);
		...
		try {
			...
			if (keyparam.equals(key)) {
				...
				for (Project project : list) {
					...
					Object projectVisitlist = productManager
							.findProjectVisitList(project);

					Contacts contacts = productManager
							.findProjectContactsList(project);
